...
Graphics::Graphics() :
    system(), bodies_settings() {}

Graphics::Graphics(double G, vector<Body> bodies) :
    system(G, 1.0 / 32.0, bodies), bodies_settings() {}

Graphics::Graphics(double G, vector<Body> bodies, int frame_rate) :
    system(G, (double) 1 / frame_rate, bodies), bodies_settings() {
    this->frame_rate = frame_rate;
}
Graphics::Graphics(double G, vector<Body> bodies, int frame_rate, double delta_time) :
        system(G, delta_time, bodies), bodies_settings() {
    this->frame_rate = frame_rate;
}
