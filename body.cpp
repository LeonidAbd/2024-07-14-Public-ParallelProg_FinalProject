...
void Body::update_acceleration(vector<Body> bodies, double delta_t, double G) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (Body body : bodies) {
            if (body.id == this->id) continue;  // the body does not have a force on itself
            double r_i = pythagoras(body.coordinate, this->coordinate);
            this->acceleration[i] += G * body.mass *
                (body.coordinate[i] - this->coordinate[i])
                / r_i / r_i / r_i;  // F = G * M * m / |r|^3 * r // a
        }
    }
}
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (Body body : bodies) {
            ...
            this->acceleration_next[i] += G * body.mass *
                (body.coordinate_next[i] - this->coordinate_next[i])
                / r_i / r_i / r_i;  // F = G * M * m / |r|^3 * r // a
        }
    }
}
